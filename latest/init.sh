...
while true; do
    ...
    if ! docker info &>/dev/null; then
        echo "======Docker 未启动,请先启动Docker======"
        exit 1  # Docker 未启动，退出脚本
    fi
    ...
    case $choice in
        ...
        2)
            echo "正在安装 内网穿透"
            bash <(curl -Ls https://alist.naspt.vip/d/shell/naspt-frp/frp.sh)
            ;;
        3)
            echo "正在安装 音乐服务"
        ...
        8)
            ...
            ;;
        9)
            echo "正在安装 docker自动更新"
            bash <(curl -Ls https://alist.naspt.vip/d/shell/naspt-docker/updk.sh)
            ;;
        b)
            echo "返回上一层。"
            ;;
